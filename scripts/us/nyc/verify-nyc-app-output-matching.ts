...
import { queryNycAppOutputFinalV4Row } from "../../../src/lib/us/us-nyc-app-output-query";
...
async function main() {
  ...
  for (const typed of ADDRESSES) {
    const norm = buildNycTruthLookupNormalizationDebug(typed.trim());
    const lineIn = norm?.normalized_full_address ?? typed.trim();
    ...
    console.log(
      `| ${typed.replace(/\|/g, "\\|")} | ${lineIn.replace(/\|/g, "\\|")} | ${debug.row_found} | ${debug.match_column ?? "none"} | ${(debug.matched_candidate ?? "").replace(/\|/g, "\\|")} | ${la.replace(/\|/g, "\\|")} | ${pa.replace(/\|/g, "\\|")} |`
    );
  }

  console.log("\n--- candidates (first address sample) ---");
  const d0 = buildNycTruthLookupNormalizationDebug(ADDRESSES[0]!.trim());
