...
-- multi_unit_transaction = TRUE only when exactly ONE DVF mutation at that grain is a multi-lot sale.
--
-- Why the old pattern failed: grouping only by address + date + price merges unrelated mutations that
-- share the same total valeur and date (or duplicate rows), so COUNT(DISTINCT unit_number) > 1 no longer
-- proves a single combined sale.
--
...
per_mutation AS (
  SELECT
    ...
    last_sale_price,
    COUNT(*) AS n,
    COUNT(DISTINCT NULLIF(TRIM(CAST(unit_number AS STRING)), '')) AS duc,
    MIN(IF(surface_m2 IS NOT NULL AND surface_m2 > 0, surface_m2, NULL)) AS min_surf,
    MAX(IF(surface_m2 IS NOT NULL AND surface_m2 > 0, surface_m2, NULL)) AS max_surf,
    COUNTIF(surface_m2 IS NOT NULL AND surface_m2 > 0) AS n_surf_pos,
...
mut_flagged AS (
  SELECT
    *,
    CASE
      WHEN duc <= 1 OR n < 2 THEN FALSE
      WHEN has_document_id AND duc > 1 THEN TRUE
      WHEN n_surf_pos >= 2 AND min_surf IS NOT NULL AND min_surf > 0 AND max_surf <= min_surf * 1.05 AND duc > 1 THEN TRUE
      ELSE FALSE
...
grain_mutation_stats AS (
  SELECT
    ...
    MAX(IF(is_multi_unit_mutation, duc, NULL)) AS distinct_unit_when_multi
  FROM mut_flagged
  GROUP BY
    postcode_norm,
    city_norm,
